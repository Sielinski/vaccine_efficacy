...
This new perspective draws attention to the fact that our model will need to accommodate for differences in population sizes. A decrease of 10,000 cases would represent a much larger change for Rhode Island than California. Each state needs its own basis to calculate the *relative* change in cases, but we also need to ensure that those relative changes are comparable: They need to be on the same scale for our model to find patterns of similarity and difference.   

While we could use the period-over-period percentage change in new cases, simple percentages  over-emphasize increases. Consider the fact that, while it's possible for increases to exceed 100%, it's *never* possible for decreases to exceed 100%. 

A more mathematically robust solution is log returns. By taking the log of the period-over-period percentage change, we overcome the inherent bias of raw percentages. 
...
# ensure training data set is balanced 
...
To explore the relationship between the availability of COVID-19 vaccines and the change in the rate of confirmed cases in the US, this analysis uses a simple linear model to predict whether a particular week's results came *before* or *after* the general availability of vaccines. 

There are multiple sources of bias in this analysis. Adjusting for them is beyond the scope of this paper, so it's important that identify the primary sources: 
